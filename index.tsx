...
// --- КОНСТАНТЫ (Всё внутри, чтобы не ломалось) ---
...
function App() {
    ...
    const downloadPDF = async (order: any) => {
        ...
        finally { setIsGeneratingPdf(false); container.innerHTML = ''; }
    };
        ...
        return (
            <div className="flex flex-col items-center justify-center min-h-screen px-10 text-center animate-fade">
                ...
                <div className="w-full space-y-4">
                    <button onClick={() => { setCity('MSK'); setView('design'); haptic(); }} className="w-full py-5 border-2 border-black rounded-2xl font-black text-lg">МОСКВА</button>
                    <button onClick={() => { setCity('SPB'); setView('design'); haptic(); }} className="w-full py-5 border-2 border-black rounded-2xl font-black text-lg">ПЕТЕРБУРГ</button>
    ...
    return (
        <div className="min-h-screen bg-white pb-32">
            <header className="sticky top-0 z-40 bg-white/90 backdrop-blur-md border-b px-6 py-4 flex justify-between items-center">
                <div className="flex items-center gap-2" onClick={() => { setView('catalog'); if(!city) setView('catalog'); }}>
                ...
                {city && <button onClick={() => { setCity(null); haptic(); }} className="text-[10px] font-black text-gray-400 uppercase bg-gray-50 px-3 py-1 rounded-full">{city} ✕</button>}
            </header>
            ...
            <main className="px-6 py-8">
                            ...
                            {CERTIFICATES[city].map((c: any) => (
                                <div key={c.id} onClick={() => { setSelectedTariff(c); setView('details'); haptic(); }} className="rounded-[2rem] border-2 border-gray-50 p-6 active:border-black transition-all">
                                    <div className="flex justify-between items-center">
                                        <div><h3 className="text-2xl font-black">{c.name}</h3><p className="text-[10px] font-black uppercase text-black/30">{c.time}</p></div>
                    ...
                    <div className="animate-fade">
                        ...
                        <div className="bg-gray-50 p-6 rounded-3xl mb-6 border">
                            {selectedTariff.includes.map((inc: string, i: number) => (
                                <p key={i} className="text-sm font-bold flex gap-3 mb-2"><span className="text-black/20">•</span>{inc}</p>
                            ))}
                        </div>
                        <button onClick={() => { setView('checkout'); haptic(); }} className="w-full py-6 bg-black text-white rounded-2xl font-black uppercase text-xs tracking-widest">Оформить за {selectedTariff.price} ₽</button>
                    </div>
                )}
                ...
                {view === 'checkout' && (
                    <div className="animate-fade max-w-sm mx-auto">
                        ...
                        <div className="space-y-4 mb-8">
                            <input value={form.sender} onChange={(e) => setForm({...form, sender: e.target.value})} placeholder="От кого" className="w-full p-5 bg-gray-50 rounded-2xl font-bold outline-none" />
                            <input value={form.recipient} onChange={(e) => setForm({...form, recipient: e.target.value})} placeholder="Кому" className="w-full p-5 bg-gray-50 rounded-2xl font-bold outline-none" />
                            <textarea value={form.message} onChange={(e) => setForm({...form, message: e.target.value})} placeholder="Пожелание" className="w-full p-5 bg-gray-50 rounded-2xl font-bold h-32 outline-none" />
                        </div>
                        <button onClick={handleBuy} className="w-full py-6 bg-black text-white rounded-2xl font-black uppercase text-xs tracking-widest">Оплатить {selectedTariff.price} ₽</button>
                    </div>
                )}
                ...
                {view === 'payment' && (
                    <div className="flex flex-col items-center justify-center py-20 animate-fade text-center">
                        ...
                        <p className="text-gray-400 text-sm mt-3 px-10">Открываем шлюз ЮKassa...</p>
                    </div>
                )}
                ...
                {view === 'success' && vault[0] && (
                    <div className="text-center py-6 animate-fade">
                        ...
                        <div className="bg-black text-white p-8 rounded-[2rem] font-mono text-3xl tracking-[0.3em] mb-8">{vault[0].id}</div>
                        <div className="space-y-4">
                            <button disabled={isGeneratingPdf} onClick={() => downloadPDF(vault[0])} className="w-full py-6 border-2 border-black rounded-2xl font-black uppercase text-xs">
                                {isGeneratingPdf ? 'Загрузка...' : 'Скачать PDF'}
                            </button>
                            <button onClick={() => notifyAdmin(vault[0])} className="w-full py-6 bg-blue-600 text-white rounded-2xl font-black uppercase text-xs">Написать админу</button>
                        </div>
                    </div>
                )}
                            ...
                            {vault.map((o: any) => (
                                <div key={o.id} className="p-8 border-2 border-gray-50 rounded-[2.5rem] bg-white shadow-sm">
                                    <div className="flex justify-between mb-4">
                                        <div><h3 className="text-2xl font-black">«{o.name}»</h3><p className="text-[10px] font-black uppercase text-black/30">{o.time} • {o.city}</p></div>
                                    ...
                                    <button disabled={isGeneratingPdf} onClick={() => downloadPDF(o)} className="w-full py-4 border-2 border-black rounded-xl font-black uppercase text-[10px]">Скачать PDF</button>
                                </div>
                            ))}
                            {vault.length === 0 && <p className="text-center text-gray-300 py-20 font-black uppercase tracking-widest">Пусто</p>}
                        </div>
                    </div>
                )}
            </main>
